...
description: "TRIGGER when the user needs to know exactly how specific code works, with every claim traceable back to the source. Use when user wants to: trace data flow, analyze algorithms, document data structures, deep-dive a function's internals, or produce technical docs where correctness matters more than readability. Every claim carries a file:line citation and the analysis ends with a mandatory verification pass. Triggers on: 'analyze code', 'codebase analysis', 'document this code', 'trace data flow', '算法分析', '代码分析', '代码梳理', '数据结构分析', '技术文档'. DO NOT trigger for: simple file reading, single-function explanation, general coding questions. USE describe-design INSTEAD when the user wants diagrams, a system overview, deployment topology, or onboarding docs for other people to read — that is documentation for humans, not line-level verification. NOTE: if the project has no CLAUDE.md, Phase 0 runs /init to generate one (~2 min, creates a new file in the repo) — tell the user this before starting."
level: 3
version: "2.1"
triggers:
  - "codebase analysis"
